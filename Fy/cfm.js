...
try {
    ...
    if (rules.length > 1 && getParam("rule") == "风影") {
        putVar("fy手动fy", "1");
        require(readFile("hiker://files/rules/FYJK/git.txt") || "https://codeberg.org/suifeng/yin/raw/branch/master/ycl.js");
        gx();
    };
    let list;
    ...
    for (let i in tit) {
        ...
        d.push({
            ...
            url: $("#noLoading#").lazyRule((li, add, up, lis, lio, los) => {
                function Snackbar() {
                    const hikerPop = $.require((readFile("hiker://files/rules/FYJK/git.txt") || "https://codeberg.org/suifeng/yin/raw/branch/master/ycl.js").replace(/[^/]*$/, "hikerPop.js"));
