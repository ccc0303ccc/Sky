function search(http, game, name, ssurl, d, page, sokey, off1, Json, type) {
    ...
    function htmlyz(MY_URL, MY_HOME, http, game, name, ssurl, d, page, sokey, nam, off1, Json, type) {
        ...
        if (ht == "" || !/电影|剧集|连续剧|电视剧|综艺|动漫|短剧|纪录|记录|番剧|搜索|视频|視頻|搜尋|電影|劇集|續集|電視劇|綜藝|動畫|短劇|紀錄|番劇/.test(ht)) {
            if (page == 1) {
    ...
    function dwlist(html, name) {
        ...
        if (/module-card-item|module-search-item/.test(html)) {
            列表 = '.module-items&&.module-search-item||.module-card-item'
        } else
        if (/product-items/.test(html)) {
            列表 = 'body&&.item'
        } else
        if (/search-list|threadlist|list-width|hl-list-item|pic-content|public-list-box/.test(html)) {
            列表 = 'body&&.hl-list-item||.search-list||.threadlist||.list-width||.pic-content||.public-list-box'
        } else
        if (/stui-vodlist|stui-vodlist__media|myui-vodlist__media|searchlilst/.test(html)) {
            列表 = '.stui-vodlist||.stui-vodlist__media||.myui-vodlist__media||.searchlilst&&li'
        } else
        if (/img-list|videoul|hl-one-list|bt_img/.test(html)) {
    ...
    function dws(name, list) {
        ...
        if (/video-serial|text-center|remarks/.test(list)) {
            更新 = '.video-serial||.text-center||.remarks&&Text'
        } else
        if (/fed-list-remarks|list-remarks/.test(list)) {
        ...
        if (/eclazy|fed-lazy|videoul-img/.test(list)) {
        ...
        if (/module-card-item-title/.test(list)) {
            片名 = '.module-card-item-title&&Text'
            链接 = 'a&&href'
        } else
        if (/thumb-txt|text-danger/.test(list)) {
            片名 = '.thumb-txt||.text-danger&&Text'
            链接 = 'a&&href'
        } else
        if (/video-serial/.test(list)) {
    ...
    if (Js) {
        ...
        if (htm == "" || !/电影|剧集|连续剧|电视剧|综艺|动漫|短剧|纪录|记录|音乐|歌|DJ|MV|番剧|漫画|说|相声|书|小品|搜索|视频|視頻|搜尋|電影|劇集|續集|電視劇|綜藝|動畫|短劇|紀錄|音樂|歌|番劇|漫畫|說|相聲|書|小藝/.test(htm) && ison != "on" || ison == "on" && (typeof(ht) == "string" ? ht.code != 200 : "")) {
            if (page == 1) {
    ...
    try {
        ...
        if (html != "" && (list == "" || list == []) && !/验证|验证码|系统安全验证|验证后|提交验证/.test(Js ? htm : ht) && (Js && status ? html.statusCode == 200 : html) && getItem("s1", "0") !== "1") {
            d.push({
                col_type: "rich_text",
            }, {
                desc: "‘‘没有找到相关内容’’",
...
type: type,
                    ...
                    let retit;
                    if (typeof(li.col_type) != "undefined" && !/^pic_3|^pic_1|^pic_2|movie_3|movie_2|avatar/.test(li.col_type)) {
                        //log("第一逻辑");
                        retit = "‘‘’’<b>" + title.replace(new RegExp(sokey), sokey.fontcolor("#FA7298")) + "</b>";
                    } else if (/avatar/.test(li.col_type)) {
                        //log("第二逻辑");
                        retit = "<b>" + title.replace(new RegExp(sokey), sokey.fontcolor("#FA7298")) + "</b>";
                    } else if (col == "movie_1_vertical_pic") {
                        //log("第三逻辑");
                        retit = "‘‘’’<b>" + title.replace(new RegExp(sokey), sokey.fontcolor("#FA7298")) + "</b>";
                    } else {
                        //log("默认逻辑");
