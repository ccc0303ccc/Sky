function yso(type) {
    ...
    let rn = type == "2" ? [getMyVar("namejs").replace(/&&.*/, "")] : off1.filter(item => (item.so != "" && item.gs != "HOST") || item.gs == "HOST").map(item => item.name.replace(/&&.*/, ""));
    let selected = type == "2" ? getMyVar("namejs").replace(/&&.*/, "") : getItem("s0", rn[0]);

    function sul(page, sokey, off1, Json, selected, file, type) {
        let names = type == "2" ? getMyVar("namejs").replace(/&&.*/, "") : off1.find(item => item.name.replace(/&&.*/, "") === selected);
        try {
            if (names) {
                ...
                try {
                    let syurl;
                    if (names.fbhost == "" || type == "2") {
                        let s = type == "2" ? getMyVar("hostjs") : names.host;
                    ...
                    ssurl = type == "2" ? rep(getMyVar("sourljs"), page, sokey, SURL, new Date().getTime()) : rep(names.sourl, page, sokey, SURL, new Date().getTime());
                } catch (e) {
                    let s = type == "2" ? getMyVar("hostjs") : names.host;
                    var SURL = getHome(s);
                    ssurl = type == "2" ? rep(getMyVar("sourljs"), page, sokey, SURL, new Date().getTime()) : rep(names.sourl, page, sokey, SURL, new Date().getTime());
    ...
    ssurl = sul(page, sokey, off1, Json, selected, file, type);
    ...
    if (page == 1) {
        ...
        s.push({
            ...
            url: $.toString((type) => {
                ...
                if (getMyVar("Mysou")) {
                    return input + $("#noLoading#").lazyRule((type) => {
                        require(config.依赖);
                        return sourl(type);
                    }, type);
                } else {
                        ...
                        let newz = list.map(li => {
                            try {
                                ...
                                return {
                                    ...
                                    desc: "年份：" + li.year,
                                    url: li.title + $("#noLoading#").lazyRule((type) => {
                                        require(config.依赖);
                                        return sourl(type)
                                    }, type),
                                    col_type: "movie_2",
    ...
    if (/cloud\.189|yun\.139|www\.123[a-zA-Z\d]{3}\.com/.test(purls)) {
        if (page == 1) {
    ...
    if (/magnet:|\.torrent|ed2k:|pan\.xunlei/.test(purls)) {
        yupan(d, /magnet:|\.torrent|ed2k:/.test(purls) ? "磁力" : "迅雷", purls, "hiker://page/diaoyong?rule=迅雷&page=fypage#" + purls);
    } else
    ...
    if (/baidu/.test(purls)) {        
        ...
        yupan(d, "百度", purls, (fetch(fxlj) == "" ? login : fxlj) + purls);
    } else
    if (getMyVar("Mysou")) {
        try {
            if (!jusou) {
                ...
                search(http, game, selected, ssurl, d, MY_PAGE, sokey, off1, Json, type);
                setResult(d);
            } else {
                //聚搜
                let 线程数 = 8;
                    ...
                    for (let k in Data) {
                        ...
                        tasks.push({
                            func: function(param) {
                                let d = [];
                                require(http + "sdw.js");
                                search(http, game, param.n, param.u, d, 1, sokey, off1, Json, type);
                                return {
                                    d: d,
                                    n: param.n
                                };
                            ...
                            param: {
                                u: sul(1, sokey, off1, Json, it, file, type),
                                n: it
                            }
                        });
                    };
        ...
        if (page == 1) {
            ...
            for (let r of list) {
                d.push({
                    ...
                    url: r.split("$$$")[1] + $('#noLoading#').lazyRule((type) => {
                        require(config.依赖);
                        return sourl(type);
                    }, type),
