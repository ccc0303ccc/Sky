function tb_url(dwfl, MY_URL) {
    ...
    if (/vodshow-(\d+)|vodshow\/(\d+)/.test(电影_url)) {
        剧集_url = 电影_url.replace(/\/20/, '/26').replace(/\/16/, '/2').replace(/\/1/, '/2').replace(/-1/, '-2');
    } else
    if (/lists\/(\d+)|list\/(\d+)|\/show\/(\d+)|tid\=(\d+)/.test(电影_url)) {
        剧集_url = 电影_url.replace(/lists\/1/, 'lists/2').replace(/list\/1/, 'list/2').replace(/show\/1/, 'show/2').replace(/tid\=1/, 'tid=2');
    } else
    if (/\/dianying|\/Movie\//.test(电影_url)) {
        剧集_url = 电影_url.replace(/\/oftensshow\/dianying/, '/oftensshow/dianshiju').replace(/\/dianying/, '/lianxuju').replace(/show\/Movie/, 'show/Tv').replace(/Movie/, 'Tv');
    } else {
    ...
    if (/vodshow-(\d+)|vodshow\/(\d+)/.test(电影_url)) {
        动漫_url = 电影_url.replace(/\/20/, '/25').replace(/\/16/, '/4').replace(/\/1/, '/4').replace(/-1/, '-4');
    } else
    if (/lists\/(\d+)|list\/(\d+)|\/show\/(\d+)|tid\=(\d+)/.test(电影_url)) {
    ...
    if (/vodshow-(\d+)|vodshow\/(\d+)/.test(电影_url)) {
        综艺_url = 电影_url.replace(/\/20/, '/21').replace(/\/16/, '/3').replace(/\/1/, '/3').replace(/-1/, '-3');
    } else
    if (/lists\/(\d+)|list\/(\d+)|\/show\/(\d+)|tid\=(\d+)/.test(电影_url)) {
...
function lei(dwfl, html) {
    if (/nav-swiper|hl-filter-list/.test(html)) {
    ...
    if (/myui-screen__list/.test(html)) {
        大类 = 'body&&.myui-nav__tabbar'
        拼接小类 = 'body&&.myui-screen__list'
        分类子 = 'body&&a:not(.text-muted):not(:matches(重置|首页|留言|最新|排行|专题|app|APP|会员|今日|更新|热榜|求片|反馈|发布|周表|地址|回家|午夜|明星|搜|榜))'
    } else
    if (/screenbox/.test(html)) {
        大类 = 'body&&.stui-header__menu'
        拼接小类 = '#screenbox&&ul'
        分类子 = 'body&&a:not(:matches(重置|首页|留言|最新|排行|专题|app|APP|会员|今日|更新|热榜|求片|反馈|发布|周表|地址|回家|午夜|明星|搜|榜))'
    } else
    if (/select-menu/.test(html)) {
...
function lieli(html) {
    ...
    if (/zmovo-teams|public-list-box|pic-content/.test(html)) {
        列表 = 'body&&.item||.public-list-box||.pic-content'
    } else
    if (/module|module-items/.test(html) && !/stui-vodlist/.test(html)) {
...
if (Js) {
    ...
    if (公用 && typeof 正文 == "string" ? 正文 == "on" : false) {
        try {
    ...
    if (html == "" || !/电影|剧集|连续剧|电视剧|综艺|动漫|短剧|纪录|记录|音乐|歌|DJ|MV|番剧|漫画|说|相声|书|小品|视频|視頻|電影|劇集|續集|電視劇|綜藝|動畫|短劇|紀錄|音樂|歌|番劇|漫畫|說|相聲|書|小藝/.test(html) && ison != "on" || ison == "on" && (typeof(ht) == "string" ? ht.code != 200 : "")) {
        if (page == 1) {
            log(html);
            toast("验证未通过或未获取到数据");
        };
