...
function html2(der, http, MY_URL, off) {
    ...
    if (ht == "" || !/电影|剧集|连续剧|电视剧|综艺|动漫|短剧|纪录|记录|番剧|视频|視頻|電影|劇集|續集|電視劇|綜藝|動畫|短劇|紀錄|番劇|集|话|章|期|回|卷|話/.test(ht)) {
        toast("没有数据可用，请切换其它源");
        log(ht);
    } else {
        html = ht;
...
function bqian(der, html) {
    ...
    if (/stui-content__detail/.test(html)) {
...
function tabs1(der, html) {
    let 线路;
    let 线路名;
    if (/product-filter-list/.test(html)) {
    ...
    if (/stui-pannel__head/.test(html)) {
        线路 = 'body&&.stui-pannel__head:not(:matches(猜你|更多|榜|屏蔽严重|友情链接))'
        线路名 = 'h3&&Text'
    } else
    if (/hl-tabs-btn/.test(html)) {
        线路 = 'body&&.hl-tabs-btn'
        线路名 = 'a&&alt'
    } else
    if (/anthology-tab/.test(html)) {
...
function tuilist(der, html) {
    ...
    if (/zmovo-team-section|public-list-box|pic-content/.test(html)) {
        推荐列表 = 'body&&.item:lt(8)||.public-list-box:lt(9)||.pic-content:lt(9)'
    } else
    if (/list-width|stui-vodlist__item|hide-b-16/.test(html)) {
...
function dw2(tuli, der) {
    ...
    if (/text-right|fed-list-remarks|list-remarks|remarks|pic-tag-left|module-item-note|module-item-text|pic-text|state|continu|public-list-subtitle|c3/.test(tuli)) {
    ...
    let 推荐图;
    if (/hl-lazy|ff-img/.test(tuli)) {
        推荐图 = '.hl-lazy||.ff-img&&data-original'
    } else
    if (/br/.test(tuli) && /eclazy/.test(tuli) || /在线电影/.test(der)) {
        推荐图 = '.br||.myui-vodlist__thumb&&style'
    } else
    if (/lazy|lazyload/.test(tuli)) {
...
if (Js) {
    ...
    if (公用 && typeof 公用.验证 == "function" && (typeof 验证 == "function" ? 验证(html) : false)) {
        try {
            公用.验证(html, MY_URL);
        } catch (e) {
            log(e.toString());
    ...
    if (html == "" || !/电影|剧集|连续剧|电视剧|综艺|动漫|短剧|纪录|记录|音乐|歌|DJ|MV|番剧|漫画|说|相声|书|小品|视频|視頻|電影|劇集|續集|電視劇|綜藝|動畫|短劇|紀錄|音樂|歌|番劇|漫畫|說|相聲|書|小藝|集|话|章|期|回|卷|話/.test(html) && ison != "on" || ison == "on" && (typeof(ht) == "string" ? ht.code != 200 : "")) {
        toast("没有数据可用，请切换其它源");
        log(html);
    } else {
        html = html;
